...
def ObjOrientation(img):
    #------------------- NOW WE FIND ORIENTATION -------------------#
        ...
        contours = cv2.findContours(mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)[1] #mask to bw if using binary & grayscale
        ...
        for i, c in enumerate(contours):
            ...
            cv2.drawContours(ori, contours, i, (0, 0, 255), 2)
            # Find the orientation of each shape
            angle = getOrientation(c, ori)
            #print("[INFO] Orientation is:", angle*(180/pi))
